def bdscan_ff_cell(X, X_index_arr=None, min_cluster_size=10, eps_l=list(range(15, 150, 5)), ftem_l=list(range(1, 60, 1)), ff_sed=123, unclustered_ratio=0.2, maxcluster_ratio=0.8):
            ...
            if len(set(label_db_new)) > 1 and len(set(label_ff_new)) > 1:
                ...
                if label_db[label_db == -1].shape[0] < np.max([X.shape[0]*unclustered_ratio, 20]):
                    ...
                    if max(Counter(label_db_new).values()) < X.shape[0]*maxcluster_ratio:
                        score_t1[str(
                            eps)] += adjusted_rand_score(label_db_new, label_ff_new)
    ...
    if max(score_t1.values()) > 0:
        fi_eps = int([k for k, v in score_t1.items()
                     if v == max(score_t1.values())][-1])
    elif max(score_t2.values()) > 0:
        fi_eps = int([k for k, v in score_t2.items()
                     if v == max(score_t2.values())][-1])
    elif max(score_t3.values()) > 0:
        fi_eps = int([k for k, v in score_t3.items()
                     if v == max(score_t3.values())][-1])
            ...
            for j in range(l_cell_clusters_df.shape[0]):
                for k in range(r_cell_clusters_df.shape[0]):
                    if j != k or l_cell_type != r_cell_type:
                        l_cell_ids_index = np.array(
                            l_cell_clusters_df.iloc[j]['cell_idx'])
                        r_cell_ids_index = np.array(
                            r_cell_clusters_df.iloc[k]['cell_idx'])

                        dis_mtx = S_all_arr[l_cell_ids_index,
                                            :][:, r_cell_ids_index]
                        ...
                        if dis_mtx[mask_close].shape[0] >= 1:
                            ...
                            S_all_arr_new[l_cell_ids_index[:, None],
                                          r_cell_ids_index] = S_all_arr[l_cell_ids_index[:, None], r_cell_ids_index]
...
def source_target_ot(dis_arr, exp_df, meta_df, known_lr_pairs, reg=1, reg_m=2, dist_cutoff=50, min_likeli_cutoff=0.01):
    ...
    mask = np.where((dis_arr == 0) | (np.isnan(dis_arr)
                    == True) | (dis_arr > dist_cutoff))
    dis_arr[mask] = np.inf
    ...
    if not (dis_arr == np.inf).all().all():
        ...
        for l_gene, r_gene in zip(l_gene_list, r_gene_list):
            ...
            S_all_arr_left = dis_arr[np.where(cell_l_exp > 0)[
                0], :][:, np.where(cell_r_exp > 0)[0]]
            exp2 = np.outer(cell_l_exp_left, cell_r_exp_left)
            S_all_arr_left = S_all_arr_left/exp2

            # remove all Inf rows and columns
            S_all_arr_left2 = S_all_arr_left[~(
                S_all_arr_left == np.inf).all(1)]
            S_all_arr_left2 = S_all_arr_left2[:, ~(
                S_all_arr_left2 == np.inf).all(0)]

            source_cell_id_left2 = source_cell_id_left[~(
                S_all_arr_left == np.inf).all(1)]
            target_cell_id_left2 = target_cell_id_left[~(
                S_all_arr_left == np.inf).all(0)]

            cell_l_exp_left2 = cell_l_exp_left[~(
                S_all_arr_left == np.inf).all(1)]
            cell_r_exp_left2 = cell_r_exp_left[~(
                S_all_arr_left == np.inf).all(0)]
            cell_l_exp_left2 = np.append(cell_l_exp_left2, 1)
            cell_r_exp_left2 = np.append(cell_r_exp_left2, 1)

            S_all_arr_left2 = np.insert(S_all_arr_left2, S_all_arr_left2.shape[0], np.ones(
                S_all_arr_left2.shape[1])*np.Inf, axis=0)
            S_all_arr_left2 = np.insert(S_all_arr_left2, S_all_arr_left2.shape[1], np.ones(
                S_all_arr_left2.shape[0])*np.Inf, axis=1)

            S_all_arr_left2[-1, -1] = 0.1

            if S_all_arr_left2.shape[0] > 1:
                # print(l_gene,r_gene)
                ga = ot.sinkhorn_unbalanced(
                    cell_l_exp_left2, cell_r_exp_left2, S_all_arr_left2, reg, reg_m)
                ga = ga/ga[-1, -1]
                ga = ga[:-1, :-1]
                # post_process
                ...
                if ga_df.shape[0] > 0:
                    # print(l_gene,r_gene)
                    ga_df['lr_pairs'] = [l_gene+"_" +
                                         r_gene for x in range(ga_df.shape[0])]
                    ga_df['l_anno'] = list(
                        meta_df.loc[list(ga_df['l_id']), 'annotation'])
                    ga_df['r_anno'] = list(
                        meta_df.loc[list(ga_df['r_id']), 'annotation'])
    ...
    for c_t_p in c_t_l:
        ...
        for lr_pair in lr_pair_l:
            ...
            if df_tmp_sel2.shape[0] >= 8:
                groupby_source = df_tmp_sel2.groupby('source_cell_idx')[
                    'likelihood'].max()
                if it_n_label == None:
                    df_all[lr_pair+"|"+label+"|"+c_t_p] = groupby_source.mean()
                else:
                    df_all[lr_pair+"|"+label+"|"+c_t_p+"|" +
                           str(it_n_label)] = groupby_source.mean()

    df_sum = pd.DataFrame(
        {'label': df_all.keys(), 'ave_likelihood': df_all.values()})
    return df_sum
...
def permutation_test(X_all, it_n=50, random_range=20):
    ...
    for it_i in range(it_n):
        ...
        random_x = np.random.choice(
            range(-1*random_range, random_range+1), size=X_all.shape[0], replace=True)
